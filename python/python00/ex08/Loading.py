def ft_tqdm(lst: range) -> None:
	...
	for x in lst:
		...
		print(f"{percentage:3}%|{(fullblocks * '█')}{blocklist[halfblock]}{' ' * (total - fullblocks)}| {x + 1}/{max_val}", end="")
		yield
	...
	print(f"100%|{((final_val + (1 if final_remain else 0)) * '█')}| {max_val}/{max_val}", end="")