...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/01/17 13:20:51 by cshi-xia          #+#    #+#             */
/*   Updated: 2023/01/17 13:20:51 by cshi-xia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// pushes from a to b
void			push_to_b(t_pushswap *stack, int start_loc,
					int end_loc, int chunk_size);
...
int				get_no_larger(t_pushswap *stack, int reference,
					int last_elem_loc);
// get number of elements smaller than reference (from top)
int				get_no_smaller(t_pushswap *stack, int reference,
					int first_elem_loc);

// determine the number of moves to sort a certain value
// determines moves needed to rotate stack_a
int				moves_needed_rot_a(t_pushswap *stack, int reference,
					int max, int min);
// calculate the value with the least moves to push into a
void			find_lesser_moves(t_pushswap *stack, int max_val,
					int min_val);
// calculate the value within max and min with the
// least amount of moves to push to b
...
void			rotate_to_correct(t_pushswap *stack,
					int moves, char stack_name);
// go to the top of the last chunk OR the bottom of the
// very first chunk (has the largest value) to sort the
// next chunk at the correct location
