#include "push_swap.h"

// void	search_and_insert(t_pushswap **dest, t_pushswap **source)
// {
// 	int	smallest_val;
// 	int	smallest_loc;
// 	t_pushswap *current; 

// 	smallest_val = INT_MAX;
// 	current = *dest;
// 	while(current)
// 	{
// 		if (current->num < smallest_val)
// 		{
// 			smallest_val = current->num;
// 		}
// 		current = current->next;
// 	}
// 	return ;
// }

int	solve_two(t_pushswap *stack)
{
	int a;
	int b;

	a = stack->stack_a[0];
	b = stack->stack_a[1];
	if (a > b)
		sa(stack);
	return (2);
}

int	solve_three(t_pushswap *stack)
{
	int a;
	int b;
	int c;

	a = stack->stack_a[0];
	b = stack->stack_a[1];
	c = stack->stack_a[2];
	if (b < c && c < a)
		ra(stack);
	else if ((b < a && c < b))
	{
		sa(stack);
		rra(stack);
	}
	else if (c < a && a < b)
		rra(stack);
	else if (a < c && b < a)
		sa(stack);
	else if (a < c && c < b)
	{
		rra(stack);
		sa(stack);
	}
	return (3);
}

int shift_to_correct(t_pushswap *stack, int larger, int toplace)
{
	if (larger)
	{
		while(toplace)
		{
			rra(stack);
			toplace--;
		}
	}
	else
	{
		while(toplace)
		{
			ra(stack);
			toplace--;
		}
	}
}

int action(t_pushswap *stack, int toplace)
{
	int mid;
	int larger;
	
	mid = stack->a_size / 2;
	larger = toplace > mid;
	// larger = if the placing value is towards the right
	if (larger)
	{
		// push the larger one to the front, then add the new one
		toplace = stack->a_size - toplace;
	}
	shift_to_correct(stack, larger, toplace);
	pa(stack);
}

//ig this is prototype algo? i think of a faster one 
//something involved with checking where is the first value's location


//aye done it 

// find the smallest variable location
int smallest_loc(int *stack, int stack_size)
{
    int smallest_val;
    int smallest_loc;
    int i;

    smallest_val = INT_MAX;
    i = -1;
    while (++i < stack_size)
    {
        if (stack[i] < smallest_val && stack[i] < smallest_val)
        {
            smallest_val = stack[i];
            smallest_loc = i;
        }
    }
    return (smallest_loc);
}

// find how many variables is the value larger than
int larger_than(int *stack, int reference, int stack_size)
{
    int	larger;
    int	i;

    larger = 0;
    i = 0;
    while(i < stack_size)
    {
        if (reference > stack[i])
            larger++;
        i++;
    }
    return (larger);
}

// determine where should we put the variable (how many steps taken)
int supposed_loc(int larger_than, int smallest_loc, int stack_size)
{
	// btw, this algo only work if stack A is already sorted
    int	suppose_loc;
    int	offset;

    offset = smallest_loc;

	// suppose_loc = smallest variable location + how many variables larger
    suppose_loc = smallest_loc + larger_than;

	// offset if the location is larger than the size (due to rotating the stack)
	// example = 5 1 2, ref = 3
	// 1 is smallest (smallest loc = 1), larger by 2
	// suppose_loc = 1 + 2 = 3, [5, 1, 2, __]
	// however, we cant use pushswap to put it at the end
	// so real_loc = found_loc - stack_size = 3 - 3 <-- (think of this like a single rotation ) = 0
	// [__, 5, 1, 2]
	// then we can rotate to put it at the back [5, 1, 2, __]
	
	while (suppose_loc >= (stack_size))
        suppose_loc = suppose_loc - stack_size;
    return (suppose_loc);
}

// this is probably the faster way to push B to A
// now need to figure out a faster way to push A to B [ current its just keep pushing, not optimised :( ]

// heads up, i dont think this works
int	alternate_valid(t_pushswap *stack)
{
	int	to_place;
	int	smlest_val_loc;
	int	second_to_place;
	int	last_to_place;

	smlest_val_loc = smallest_loc(stack->stack_a, stack->a_size);
	to_place = supposed_loc(larger_than(stack->stack_a, stack->stack_b[0], stack->a_size), smlest_val_loc, stack->a_size);
	second_to_place = supposed_loc(larger_than(stack->stack_a, stack->stack_b[1], stack->a_size), smlest_val_loc, stack->a_size);
	last_to_place = supposed_loc(larger_than(stack->stack_a, stack->stack_b[stack->b_size - 1], stack->a_size), smlest_val_loc, stack->a_size);
	if (stack->b_size != 1)
	{
		if ((second_to_place + 1) < to_place)
		{
			sb(stack);
			if (stack->b_size <= (stack->total_size / 2) && (last_to_place + 2) < (second_to_place + 1))
				rrb(stack);
			return (1);
		}
		else if (stack->b_size <= (stack->total_size / 2) && (last_to_place + 1) < to_place)
		{
			rrb(stack);
			return (1);
		}
	}
	return (0);	
}

void	search_and_add(t_pushswap *stack)
{
	int	to_place;
	int smlest_val_loc;

	smlest_val_loc = smallest_loc(stack->stack_a, stack->a_size);
	to_place = supposed_loc(larger_than(stack->stack_a, stack->stack_b[0], stack->a_size), smlest_val_loc, stack->a_size);
	if (alternate_valid(stack))
		return ;
	action(stack, to_place);
}

// holy shit it worked wtf
int	solve_more(t_pushswap *stack)
{
	int smlest_val_loc;
	int larger;

	better_pb(stack);
	
	if (stack->a_size == 3)
		solve_three(stack);
	else if (stack->a_size == 2)
		solve_two(stack);
	while(stack->b_size > 0)
	{
		search_and_add(stack);
	}

	smlest_val_loc = smallest_loc(stack->stack_a, stack->a_size);

	// if the smallest value location is at the right side of the stack
	larger = smlest_val_loc >= (stack->a_size / 2);
	if (larger)
		// then instead of shifting first to last, shift last to first (faster)
		smlest_val_loc = stack->a_size - smlest_val_loc;
	shift_to_correct(stack, larger, smlest_val_loc);
	return(34);
}
