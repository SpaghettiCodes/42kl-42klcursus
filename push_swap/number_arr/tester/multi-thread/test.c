...
void	gen_num(int write_file, int write_pipe, char **envp, int count, int seed, int len)
{
	...
	close(write_pipe);
	if(execve("../to_submit/pushswap", args, envp) == -1)
...
int	stats(int cases, int cases_len)
{
	...
	resultfd = open("step_taken.log", O_RDONLY);
	statfd = open("stats.log", O_WRONLY | O_TRUNC | O_CREAT, 0777);
...
int main(int ac, char **av, char **envp)
{
	...
	if (access("../to_submit/", F_OK))
		return(printf("folder is gone\n"));

	system("cd ../to_submit && make");

	if (access("../to_submit/pushswap", X_OK))
	...
	test_num = open("test_nums.log", O_WRONLY | O_TRUNC | O_CREAT, 0777);
	step_taken = open("step_taken.log", O_WRONLY | O_TRUNC | O_CREAT, 0777);
